...
from mata.ta_config.general_config import task_assign_scheme, build_agents_and_tasks, build_obj_pos_large


class LMTAPolicy(object):
    def __init__(self):
        self.str = 'lmta'
    ...
    # Distance and path information from UAV to each target, and between targets
    ...
    def update_current_score(self):
        if len(self.p) > 1:
            if len(self.p) > 2 and self.scale <= 10:
                dist = self.dis_t2t[self.p[-2]][self.p[-1]]
                self.dist_cost += dist
            else:
                self.dist_cost += self.dis_t2t[self.p[-2]][self.p[-1]]
        else:
            self.dist_cost += self.dis_u2t[0][self.p[0]]

        self.dist_end = self.dis_t2e[0][self.p[-1]]

    def score_scheme(self, j):
        # Reward after adding the task
        distance_j = self.dist_cost
        if self.p:
            if len(self.p) >= 2 and self.scale <= 10:
                dist = self.dis_t2t[self.p[-1]][j]
                distance_j += dist
            else:
                distance_j += self.dis_t2t[self.p[-1]][j]
        else:
            distance_j += self.dis_u2t[0][j]
        w_aj = (self.Lambda ** distance_j)  # * self.c_bar[j]
        f_dist = distance_j * self.scale + self.dis_t2e[0][j] * self.scale
        if f_dist > self.L_dist and self.L_dist < 1000:
            w_aj = 0.001
        return w_aj
    ...
    def update_score(self):
        ...
        if len(self.p) > 0:
            ...
            for p_idx in range(len(self.p) - 1):
                if p_idx >= 1 and self.scale <= 10:
                    # key = str((self.p[p_idx - 1], self.p[p_idx], self.p[p_idx + 1]))
                    dist = self.dis_t2t[self.p[p_idx]][self.p[p_idx + 1]]       # + self.dist_turning[key]
                    distance_j += dist
                else:
            ...
            if is_cross:
                ...
                if d2 < d1:
                    ...
                    if a_star == self.id:
                        ...
                        self.dist_cost = self.update_score()
                        self.wa_star = 0.0
                        self.update_wa_ja()
                    elif a_par == self.id:
...
def main(envs, agents, task_area, tars_pos, scl):
    ...
    tar_ids, res, allocost, dists, all_paths = task_assign_scheme(agents, tars_pos, scl, LMTAPolicy, name)

    # print("Time used [sec]: ", allocost)
    # print("all_rewards: ", res)
    # print(dists)
    print("max and sum travel distance: ", max(dists), sum(dists) / 1000)
