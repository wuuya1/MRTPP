...
def png_to_gif(general_fig_name, last_fig_name, animation_filename, video_filename):
    ...
    # Dump all those images into a list (sorted by timestep)
    filenames = glob.glob(all_filenames)
    filenames.sort()
    images = []
    for filename in filenames:
        images.append(imageio.imread(filename))  # 使用修正后的imageio
        os.remove(filename)  # 注意：这会删除原始图片文件，确保这是你想要的操作
    # Append the last figure multiple times
    for _ in range(10):  # 使用下划线作为循环变量，因为它不会被使用
        images.append(imageio.imread(last_filename))

    # Save the images as a gif in a new animations sub-folder
    ...
    imageio.mimsave(animation_filename, images, loop=0)

    # Convert the .gif to .mp4
    clip = VideoFileClip(animation_filename)
